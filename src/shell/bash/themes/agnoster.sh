...
ONLINE="$\033[0;32m ◉ \033[0m"
OFFLINE="$\033[0;31m ⦿ \033[0m"
...
prompt_battery() {
  ...
  if [[ $(uname) == "Darwin" ]] ; then
    ...
    i=$(ioreg -n AppleSmartBattery -r | awk '$1~/Capacity/{c[$1]=$3} END{OFMT="%.0f"; max=c["\"MaxCapacity\""]; print (max>0? 100*c["\"CurrentCapacity\""]/max: "?")}'
)
      echo $i
    }

    i=$(ioreg -n AppleSmartBattery -r | awk '$1~/CurrentCapacity/{c[$1]=$3} END{print (c["\"CurrentCapacity\""])}'
)
    ...
    if [[ $(ioreg -rc AppleSmartBattery | grep -c '^.*"ExternalConnected"\ =\ No') -eq 1 ]] ; then
      if [ $b -gt 50 ] ; then
        prompt_segment green white "$HEART$(battery_pct_remaining)"
      elif [ $b -gt 20 ] ; then
        prompt_segment yellow white "$HEART$(battery_pct_remaining)"
      else
        prompt_segment red white "$HEART$(battery_pct_remaining)"
      fi
    fi
  fi
...
function prompt_online() {
  if [[ -f ~/.offline ]]; then
    prompt_segment black red $OFFLINE
  else
    prompt_segment black cyan $ONLINE
  fi
}
